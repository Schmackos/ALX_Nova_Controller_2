// test_hal_es9840.cpp
// Tests a mock HalEs9840 class modelling the ESS ES9840 I2C ADC.
//
// The ES9840 is an ESS SABRE 4-channel ADC (116dB DNR):
//   - I2C address 0x40 (configurable via address pins)
//   - 4-channel ADC with TDM output
//   - Chip ID: 0x87 (register 0xE1)
//   - Architecturally identical to ES9842PRO (same register map), lower DNR spec
...
// ===== Mock ES9840 ADC device =====
class HalEs9840Mock : public HalAudioDevice, public HalAudioAdcInterface {
public:
    static const uint8_t  kI2cAddr  = 0x40;
    static const uint8_t  kChipId   = 0x87;
    ...
    bool     _muted        = false;
    uint8_t  _volume       = 100;
    uint16_t _chVol16[4]   = {0x7FFF, 0x7FFF, 0x7FFF, 0x7FFF};
    ...
    HalEs9840Mock() {
        strncpy(_descriptor.compatible, "ess,es9840", 31);
        _descriptor.compatible[31] = '\0';
        strncpy(_descriptor.name, "ES9840", 32);
...
static HalEs9840Mock* adc;

void setUp(void) {
    WireMock::reset();
    adc = new HalEs9840Mock();
}
...
void test_descriptor_compatible_string(void) {
    TEST_ASSERT_EQUAL_STRING("ess,es9840", adc->getDescriptor().compatible);
}
...
void test_descriptor_chip_id_constant(void) {
    // ES9840 chip ID is 0x87 (distinct from ES9842PRO=0x83, ES9843PRO=0x8F, ES9841=0x91)
    TEST_ASSERT_EQUAL_HEX8(0x87, HalEs9840Mock::kChipId);
}
...
void test_input_source_name_pair_a(void) {
    ...
    const AudioInputSource* src = adc->getInputSource();
    TEST_ASSERT_NOT_NULL(src);
    TEST_ASSERT_EQUAL_STRING("ES9840 CH1/2", src->name);
}
...
void test_input_source_at_1_is_pair_b(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("ES9840 CH3/4", srcB->name);
