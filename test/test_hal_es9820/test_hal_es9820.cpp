// test_hal_es9820.cpp
// Tests a mock HalEs9820 class modelling the ESS ES9820 I2C ADC.
//
// The ES9820 is an ESS SABRE ADC with I2C control interface:
//   - I2C address 0x40 (configurable via address pins)
//   - 2-channel (stereo) ADC, chip ID 0x84
//   - Supported sample rates: 44100, 48000, 96000, 192000 Hz
//   - Hardware volume (16-bit per channel)
//   - DATA_GAIN per channel: 0, 6, 12, 18 dB (18 dB max; values above rejected)
//   - HPF via DC-blocking register (0x65 bit2 / 0x76 bit2, same as ES9822PRO)
//   - Per-channel filter preset (0-7) applied to both channels simultaneously
...
//   - adcSetGain(): valid 0,6,12,18; 19+ rejected
//   - adcSetHpfEnabled(): toggles HPF state, always succeeds
...
// ===== Mock ES9820 ADC device =====
//
// Models the ESS ES9820 stereo ADC with I2C control interface.
// DATA_GAIN max is 18 dB (2-bit register, values 0-3). Values >18 are rejected.
// HPF is register-controlled (same layout as ES9822PRO).
// Filter presets 0-7 valid (applied to both CH1 and CH2).
// Supported rates: 44100, 48000, 96000, 192000 Hz.
//
class HalEs9820Mock : public HalAudioDevice, public HalAudioAdcInterface {
public:
    ...
    HalEs9820Mock() {
        strncpy(_descriptor.compatible, "ess,es9820", 31);
        _descriptor.compatible[31] = '\0';
        strncpy(_descriptor.name, "ES9820", 32);
    ...
    bool adcSetGain(uint8_t dB) override {
        // ES9820: DATA_GAIN is 2-bit, valid steps: 0, 6, 12, 18 dB (18 max)
        if (dB > 18) return false;
        uint8_t gainReg = dB / 6;   // floor: 0→0, 6→1, 7-12→2, 13-18→3
...
// ===== Fixtures =====
static HalEs9820Mock* adc;

void setUp(void) {
    WireMock::reset();
    adc = new HalEs9820Mock();
}
...
// ----- 1. descriptor compatible string is "ess,es9820" -----
void test_descriptor_compatible_string(void) {
    TEST_ASSERT_EQUAL_STRING("ess,es9820", adc->getDescriptor().compatible);
}
...
// ----- 16. input source name matches descriptor name "ES9820" -----
void test_input_source_name_matches_descriptor(void) {
    adc->init();
    const AudioInputSource* src = adc->getInputSource();
    TEST_ASSERT_NOT_NULL(src);
    TEST_ASSERT_EQUAL_STRING("ES9820", src->name);
}
...
// ==========================================================================
// Section 4: Gain tests (0-18 dB in 6 dB steps; values >18 rejected)
// ==========================================================================
...
// ----- 24. adcSetGain(19) is rejected (above 18 dB max) -----
void test_gain_above_max_rejected(void) {
    TEST_ASSERT_FALSE(adc->adcSetGain(19));
...
// ----- 28. adcSetHpfEnabled(true) succeeds -----
...
// ----- 29. adcSetHpfEnabled(false) succeeds -----
...
// ----- 35. adcGetSampleRate() returns the set rate -----
...
// ----- 36. setFilterPreset(0-7) all succeed -----
...
// ----- 37. setFilterPreset(8) returns false -----
...
// ----- 38. setMute(true) and setMute(false) succeed -----
...
// ----- 39. configure(48000, 32) succeeds -----
...
// ----- 40. configure(44100, 24) succeeds -----
...
// ----- 41. configure(96000, 24) succeeds -----
