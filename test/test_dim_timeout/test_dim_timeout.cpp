...
static void sim_screen_dim(void) {
    ...
    sim_set_backlight(BL_BRIGHTNESS_DIM);
}
...
static void sim_tick(void) {
    ...
    if (sim_screen_awake && !sim_screen_dimmed && dim_ms > 0) {
...
void test_dimTimeout_default_is_zero(void) {
    TEST_ASSERT_EQUAL_UINT32(0, appState.dimTimeout);
}
...
void test_dimTimeout_valid_values(void) {
    unsigned long valid[] = {0, 5000, 10000, 30000, 60000};
    for (int i = 0; i < 5; i++) {
...
void test_dim_triggers_after_timeout(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8(BL_BRIGHTNESS_DIM, sim_backlight_level);
}

void test_dim_disabled_when_zero(void) {
    appState.setDimTimeout(0);
    appState.screenTimeout = 0;  // Also disable sleep so screen stays on
...
int main(int argc, char **argv) {
    ...
    // AppState tests
    RUN_TEST(test_dimTimeout_default_is_zero);
    ...
    RUN_TEST(test_dim_disabled_when_zero);
