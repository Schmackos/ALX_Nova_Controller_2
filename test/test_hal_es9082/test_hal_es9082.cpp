// test_hal_es9082.cpp
// Tests a mock HalEs9082 class modelling the ESS ES9082 8-channel I2C DAC.
//
// The ES9082 is ESS's HyperStream IV 8-channel DAC in a 48-pin QFN package:
//   - I2C address 0x48
//   - 8-channel (TDM) DAC, exposed as 4 stereo sink pairs
//   - 120dB DNR, PCM up to 768kHz
//   - Larger package than ES9081; additional GPIO and optional ASP2 interface
...
// ===== Mock ES9082 8-channel DAC =====

class HalEs9082Mock : public HalAudioDevice, public HalAudioDacInterface {
public:
    ...
    HalEs9082Mock() {
        ...
        strncpy(_descriptor.compatible, "ess,es9082", 31);
        strncpy(_descriptor.name, "ES9082", 32);
...
// ===== Fixtures =====
static HalEs9082Mock* dac;

void setUp(void) {
    WireMock::reset();
    dac = new HalEs9082Mock();
}
...
void test_descriptor_compatible_string(void) {
    TEST_ASSERT_EQUAL_STRING("ess,es9082", dac->getDescriptor().compatible);
}

void test_descriptor_name(void) {
    TEST_ASSERT_EQUAL_STRING("ES9082", dac->getDescriptor().name);
}
...
void test_dac_set_volume(void) {
    dac->init();
    TEST_ASSERT_TRUE(dac->dacSetVolume(80));
...
void test_dac_set_sample_rate(void) {
    TEST_ASSERT_TRUE(dac->dacSetSampleRate(768000));
    TEST_ASSERT_EQUAL(768000u, dac->dacGetSampleRate());
}
