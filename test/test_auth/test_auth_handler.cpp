...
String getWebPassword() {
    return mockWebPassword;
}

// Set web password
void setWebPassword(String newPassword) {
    mockWebPassword = newPassword;
}
...
void test_session_creation_empty_slot(void) {
    ...
    TEST_ASSERT_EQUAL_STRING(sessionId.c_str(), activeSessions[0].sessionId.c_str());
}
...
void test_session_creation_fills_slots(void) {
    ...
    for (int i = 0; i < MAX_SESSIONS; i++) {
        TEST_ASSERT_EQUAL_STRING(sessionIds[i].c_str(), activeSessions[i].sessionId.c_str());
    }
}
...
void test_session_creation_full_eviction(void) {
    ...
    TEST_ASSERT_EQUAL_STRING(sessionIds[5].c_str(), activeSessions[0].sessionId.c_str());
}
...
void test_login_success(void) {
    ...
    if (strcmp("correct_password", "correct_password") == 0) {
...
void test_login_failure(void) {
    ...
    if (strcmp("wrong_password", "correct_password") != 0) {
        // Login should fail (no session created)
        TEST_ASSERT_FALSE(true); // This would be part of the API handler
    }
}
...
void test_multiple_sessions_independent_validation(void) {
    ...
    // Second should still be valid (created later)
