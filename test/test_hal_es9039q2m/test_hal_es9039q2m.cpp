// test_hal_es9039q2m.cpp
// Tests a mock HalEs9039Q2M class modelling the ESS ES9039Q2M I2C DAC.
//
// The ES9039Q2M is a flagship ESS SABRE DAC (Hyperstream IV) with I2C control:
//   - I2C address 0x48 (configurable via address pins)
//   - 2-channel (stereo) DAC
//   - 130dB DNR (improved over ES9038Q2M), PCM up to 768kHz, DSD1024
//   - 128-step hardware volume attenuation (0.5dB per step)
//   - 8 digital filter presets (0-7), including 2 Hyperstream IV hybrid modes
...
//   - NO ADC path, NO PGA, NO HPF control, NO MQA, NO LINE_DRIVER
...
// ===== Mock ES9039Q2M DAC device =====
//
// Models the ESS ES9039Q2M stereo DAC with I2C control interface.
// Hyperstream IV architecture: 130dB DNR, DSD1024 capable.
// Volume uses inverted attenuation: 100% -> 0x00 (no attenuation), 0% -> 0xFF (full mute).
// Filter presets 0-7 valid; presets 6 and 7 are Hyperstream IV hybrid modes.
// Supported rates: 44100, 48000, 96000, 192000, 384000, 768000 Hz.
//
class HalEs9039Q2MMock : public HalAudioDevice, public HalAudioDacInterface {
public:
    ...
    HalEs9039Q2MMock() {
        strncpy(_descriptor.compatible, "ess,es9039q2m", 31);
        _descriptor.compatible[31] = '\0';
        strncpy(_descriptor.name, "ES9039Q2M", 32);
...
// ===== Fixtures =====
static HalEs9039Q2MMock* dac;

void setUp(void) {
    WireMock::reset();
    dac = new HalEs9039Q2MMock();
}
...
// ----- 1. descriptor compatible string is "ess,es9039q2m" -----
void test_descriptor_compatible_string(void) {
    TEST_ASSERT_EQUAL_STRING("ess,es9039q2m", dac->getDescriptor().compatible);
}
...
void test_descriptor_capabilities(void) {
    ...
    // DAC-only: no ADC path, no PGA, no HPF, no MQA, no line driver
    ...
    TEST_ASSERT_FALSE(caps & HAL_CAP_LINE_DRIVER);
}
...
// ----- 23b. Hyperstream IV hybrid filter presets 6 and 7 are valid -----
void test_filter_preset_hyperstream_iv_hybrid(void) {
    // Preset 6 = Hybrid Minimum Phase (Hyperstream IV)
    TEST_ASSERT_TRUE(dac->setFilterPreset(6));
    TEST_ASSERT_EQUAL(6, dac->_filterPreset);
    // Preset 7 = Hybrid Linear Phase (Hyperstream IV)
    TEST_ASSERT_TRUE(dac->setFilterPreset(7));
    TEST_ASSERT_EQUAL(7, dac->_filterPreset);
}
...
void test_build_sink_name(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("ES9039Q2M", out.name);
}
...
int main(int argc, char** argv) {
    ...
    RUN_TEST(test_filter_preset_hyperstream_iv_hybrid);
