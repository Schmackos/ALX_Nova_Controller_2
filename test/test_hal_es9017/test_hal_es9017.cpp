// test_hal_es9017.cpp
// Tests a mock HalEs9017 class modelling the ESS ES9017 8-channel I2C DAC.
//
// The ES9017 is ESS's HyperStream IV 8-channel DAC:
//   - I2C address 0x48
//   - 8-channel (TDM) DAC, exposed as 4 stereo sink pairs
//   - 120dB DNR, PCM up to 768kHz
//   - Pin-compatible drop-in replacement for ES9027PRO
//   - Register map identical to ES9027PRO including TDM channel mapping (0x40-0x47)
//   - getSinkCount() returns 4 after init
...
// ===== Mock ES9017 8-channel DAC =====

class HalEs9017Mock : public HalAudioDevice, public HalAudioDacInterface {
public:
    ...
    HalEs9017Mock() {
        ...
        strncpy(_descriptor.compatible, "ess,es9017", 31);
        strncpy(_descriptor.name, "ES9017", 32);
...
// ===== Fixtures =====
static HalEs9017Mock* dac;

void setUp(void) {
    WireMock::reset();
    dac = new HalEs9017Mock();
}
...
void test_descriptor_compatible_string(void) {
    TEST_ASSERT_EQUAL_STRING("ess,es9017", dac->getDescriptor().compatible);
}

void test_descriptor_name(void) {
    TEST_ASSERT_EQUAL_STRING("ES9017", dac->getDescriptor().name);
}
...
// ==========================================================================
// Section 6: Sample rate and bit depth
// ==========================================================================

void test_sample_rates_all_supported(void) {
...
void test_sample_rate_unsupported(void) {
    TEST_ASSERT_FALSE(dac->configure(32000, 32));
    TEST_ASSERT_EQUAL(48000u, dac->_sampleRate);
}

void test_configure_bit_depths(void) {
...
void test_configure_unsupported_bits(void) {
    TEST_ASSERT_FALSE(dac->configure(48000, 20));
    TEST_ASSERT_EQUAL(32, dac->_bitDepth);
}

// ==========================================================================
// Section 7: DAC interface delegation
// ==========================================================================

void test_dac_set_volume(void) {
    dac->init();
    TEST_ASSERT_TRUE(dac->dacSetVolume(30));
    TEST_ASSERT_EQUAL(30, dac->dacGetVolume());
}
...
int main(int argc, char** argv) {
    ...
    // Section 6: Sample rate / bit depth
    RUN_TEST(test_sample_rates_all_supported);
    RUN_TEST(test_sample_rate_unsupported);
    RUN_TEST(test_configure_bit_depths);
    RUN_TEST(test_configure_unsupported_bits);

    // Section 7: DAC interface
