// test_hal_cs43131.cpp
// Tests a mock HalCs43131 class modelling the Cirrus Logic CS43131 I2C DAC.
//
// The CS43131 is a high-performance Cirrus Logic MasterHIFI DAC with
// integrated headphone amplifier:
//   - I2C address 0x48
//   - 2-channel (stereo) DAC WITH integrated headphone amplifier
//   - 127dB dynamic range unweighted
//   - -115dB THD+N
//   - 32-bit, PCM up to 384kHz, DSD256
//   - 2 VRMS into 600 ohm headphones
...
//   - setFilterPreset(): accepts 0-6; rejects >= 7 (CS43131 has 7 presets)
...
// ===== Mock CS43131 DAC device =====
//
// Models the Cirrus Logic CS43131 stereo DAC with integrated headphone
// amplifier and I2C control interface. Volume uses percentage scaling:
// 100% = 0dB, 0% = full attenuation. Filter presets 0-6 valid (7 presets
// total). Supported rates: 44100, 48000, 96000, 192000, 384000 Hz.
// Does NOT support 768kHz (unlike ESS SABRE DACs). Headphone amplifier
// can be enabled/disabled independently of DAC operation.
//
class HalCs43131Mock : public HalAudioDevice, public HalAudioDacInterface {
public:
    ...
    HalCs43131Mock() {
        strncpy(_descriptor.compatible, "cirrus,cs43131", 31);
        _descriptor.compatible[31] = '\0';
        strncpy(_descriptor.name, "CS43131", 32);
    ...
    bool configure(uint32_t rate, uint8_t bits) override {
        // CS43131 supports up to 384kHz — NO 768kHz
    ...
    bool setFilterPreset(uint8_t preset) {
        if (preset >= 7) return false;   // CS43131 has 7 presets (0-6)
...
// ===== Fixtures =====
static HalCs43131Mock* dac;

void setUp(void) {
    WireMock::reset();
    dac = new HalCs43131Mock();
}
...
// ----- 1. descriptor compatible string is "cirrus,cs43131" -----
void test_descriptor_compatible_string(void) {
    TEST_ASSERT_EQUAL_STRING("cirrus,cs43131", dac->getDescriptor().compatible);
}
...
// ----- 30. configure(768000, 32) rejected (CS43131 max is 384kHz) -----
...
// ----- 40. dacSetSampleRate(768000) rejected (CS43131 max 384kHz) -----
...
void test_build_sink_populates_name(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("CS43131", out.name);
}
...
// ----- 53 (test 52). HP amp state survives reconfigure -----
