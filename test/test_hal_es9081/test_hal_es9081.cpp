// test_hal_es9081.cpp
// Tests a mock HalEs9081 class modelling the ESS ES9081 8-channel I2C DAC.
//
// The ES9081 is ESS's HyperStream IV 8-channel DAC in a 40-pin QFN package:
//   - I2C address 0x48
...
// ===== Mock ES9081 8-channel DAC =====

class HalEs9081Mock : public HalAudioDevice, public HalAudioDacInterface {
public:
    ...
    HalEs9081Mock() {
        ...
        strncpy(_descriptor.compatible, "ess,es9081", 31);
        strncpy(_descriptor.name, "ES9081", 32);
...
// ===== Fixtures =====
static HalEs9081Mock* dac;

void setUp(void) {
    WireMock::reset();
    dac = new HalEs9081Mock();
}
...
void test_descriptor_compatible_string(void) {
    TEST_ASSERT_EQUAL_STRING("ess,es9081", dac->getDescriptor().compatible);
}

void test_descriptor_name(void) {
    TEST_ASSERT_EQUAL_STRING("ES9081", dac->getDescriptor().name);
}
...
void test_dac_set_sample_rate(void) {
    TEST_ASSERT_TRUE(dac->dacSetSampleRate(384000));
