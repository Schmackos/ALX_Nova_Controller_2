// test_hal_es9027pro.cpp
// Tests a mock HalEs9027pro class modelling the ESS ES9027PRO 8-channel I2C DAC.
//
// The ES9027PRO is ESS's 8-channel HyperStream IV DAC:
//   - I2C address 0x48
//   - 8-channel (TDM) DAC, exposed as 4 stereo sink pairs
//   - 124dB DNR, PCM up to 768kHz
//   - 8-bit per-channel hardware volume attenuation
//   - 8 digital filter presets
...
// ===== Mock ES9027PRO 8-channel DAC =====

class HalEs9027ProMock : public HalAudioDevice, public HalAudioDacInterface {
public:
    ...
    HalEs9027ProMock() {
        ...
        strncpy(_descriptor.compatible, "ess,es9027pro", 31);
        strncpy(_descriptor.name, "ES9027PRO", 32);
    ...
    bool buildSinkAt(int idx, uint8_t sinkSlot, AudioOutputSink* out) override {
        ...
        static const char* names[4] = {
            "ES9027PRO CH1/2", "ES9027PRO CH3/4",
...
// ===== Fixtures =====
static HalEs9027ProMock* dac;

void setUp(void) {
    WireMock::reset();
    dac = new HalEs9027ProMock();
}
...
void test_descriptor_compatible_string(void) {
    TEST_ASSERT_EQUAL_STRING("ess,es9027pro", dac->getDescriptor().compatible);
}

void test_descriptor_name(void) {
    TEST_ASSERT_EQUAL_STRING("ES9027PRO", dac->getDescriptor().name);
}
...
void test_dac_set_sample_rate(void) {
    TEST_ASSERT_TRUE(dac->dacSetSampleRate(192000));
    TEST_ASSERT_EQUAL(192000u, dac->dacGetSampleRate());
}

// ==========================================================================
// Section 8: Multi-sink tests (8ch TDM — 4 stereo pairs)
// ==========================================================================
