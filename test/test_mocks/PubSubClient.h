...
public:
    ...
    PubSubClient(WiFiClient& wifiClient) : client(&wifiClient), callback(nullptr) {}
    ...
    bool unsubscribe(const char* topic) {
        ...
        std::string topicStr(topic);
        for (auto it = subscribedTopics.begin(); it != subscribedTopics.end(); ++it) {
            if (*it == topicStr) {
    ...
    static bool wasMessagePublished(const char* topic) {
        return publishedMessages.find(std::string(topic)) != publishedMessages.end();
    }
