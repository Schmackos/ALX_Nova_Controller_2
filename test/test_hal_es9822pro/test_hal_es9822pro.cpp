// test_hal_es9822pro.cpp
// Tests a mock HalEs9822Pro class modelling the ESS ES9822PRO I2C ADC.
//
// The ES9822PRO is a high-performance ESS SABRE ADC with I2C control:
//   - I2C address 0x40 (configurable via address pins)
//   - 2-channel (stereo) ADC
//   - Supported sample rates: 48kHz, 96kHz, 192kHz, 384kHz
//   - Hardware volume (attenuation) via I2C register
//   - Gain settings: 0, 6, 12, 18 dB (18 dB max, values above clamped)
//   - HPF enable/disable via I2C
...
//   - adcSetSampleRate(): accepts 48000, 96000, 192000, 384000; rejects others
//   - setFilterPreset(): accepts 0-7; rejects >=8
//   - setVolume(): 0-100% scaling
//   - setMute(): toggles mute flag
//   - configure(): accepts 48000/96000/192000/384000 with 16/24/32 bit depth
//   - type = HAL_DEV_ADC
//   - bus = HAL_BUS_I2C
//   - capabilities includes HAL_CAP_ADC_PATH, HAL_CAP_PGA_CONTROL, HAL_CAP_HPF_CONTROL, HAL_CAP_HW_VOLUME
//   - NO DAC path
...
// ===== Mock ES9822PRO ADC device =====
//
// Models the ESS ES9822PRO stereo ADC with I2C control interface.
// Gain is clamped to 18 dB maximum. Filter presets 0-7 valid.
// Supported rates: 48000, 96000, 192000, 384000 Hz.
//
class HalEs9822ProMock : public HalAudioDevice, public HalAudioAdcInterface {
public:
    ...
    HalEs9822ProMock() {
        strncpy(_descriptor.compatible, "ess,es9822pro", 31);
        _descriptor.compatible[31] = '\0';
        strncpy(_descriptor.name, "ES9822PRO", 32);
        ...
        _descriptor.sampleRatesMask = HAL_RATE_48K | HAL_RATE_96K | HAL_RATE_192K;
    ...
    bool configure(uint32_t rate, uint8_t bits) override {
        if (rate != 48000 && rate != 96000 && rate != 192000 && rate != 384000) return false;
    ...
    bool adcSetGain(uint8_t dB) override {
        // Clamp to 18 dB maximum
        if (dB > 18) dB = 18;
        // Only discrete steps accepted: 0, 6, 12, 18
    ...
    bool adcSetSampleRate(uint32_t hz) override {
        if (hz != 48000 && hz != 96000 && hz != 192000 && hz != 384000) return false;
        _sampleRate = hz;
        return true;
    }
    ...
    // ----- ES9822PRO-specific: filter preset -----
...
// ===== Fixtures =====
static HalEs9822ProMock* adc;

void setUp(void) {
    WireMock::reset();
    adc = new HalEs9822ProMock();
}
...
// ----- 1. descriptor compatible string is "ess,es9822pro" -----
void test_descriptor_compatible_string(void) {
    TEST_ASSERT_EQUAL_STRING("ess,es9822pro", adc->getDescriptor().compatible);
}
...
// ----- 3. capabilities include HAL_CAP_ADC_PATH -----
...
// ----- 15. input source name matches descriptor name "ES9822PRO" -----
void test_input_source_name_matches_descriptor(void) {
    adc->init();
    const AudioInputSource* src = adc->getInputSource();
    TEST_ASSERT_NOT_NULL(src);
    TEST_ASSERT_EQUAL_STRING("ES9822PRO", src->name);
}
...
void test_volume_100_percent(void) {
    adc->_volume = 0;  // Start from a different value
...
// ----- 24. adcSetHpfEnabled(true) succeeds -----
...
// ----- 25. adcSetHpfEnabled(false) succeeds -----
...
// ----- 28c. adcSetSampleRate(384000) succeeds -----
void test_sample_rate_384k(void) {
    TEST_ASSERT_TRUE(adc->adcSetSampleRate(384000));
    TEST_ASSERT_EQUAL(384000u, adc->_sampleRate);
}

// ----- 28d. adcSetSampleRate(44100) returns false (not supported) -----
void test_sample_rate_44k1_rejected(void) {
    TEST_ASSERT_FALSE(adc->adcSetSampleRate(44100));
}
...
// ----- 33b. configure(96000, 24) succeeds -----
...
int main(int argc, char** argv) {
    ...
    RUN_TEST(test_gain_invalid_discrete_values);
    ...
    RUN_TEST(test_sample_rate_44k1_rejected);
