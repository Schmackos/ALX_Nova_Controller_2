...
This section also covers on-device testing — hardware-in-the-loop validation using the health check module and a Python pytest harness running against a real ESP32-P4 board. See the sub-pages for details:

- [On-Device Testing](./device-testing) — the `health_check` module, two-phase boot, check categories, and the pytest harness
- [CI Device Tests](./ci-device-tests) — self-hosted runner setup, workflow triggers, and automated issue creation
...
These gaps are addressed by the on-device testing harness described in the [On-Device Testing](./device-testing) and [CI Device Tests](./ci-device-tests) pages.
