...
class TestDacEeprom:
    ...
    def test_eeprom_read_returns_json(self, api):
        """GET /api/dac/eeprom returns valid JSON."""
        resp = api.get("/api/dac/eeprom")
        assert resp.status_code == 200
        data = resp.json()
        assert isinstance(data, dict)

    def test_eeprom_scan(self, api):
        """POST /api/dac/eeprom/scan re-scans I2C + EEPROM."""
        resp = api.post("/api/dac/eeprom/scan")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_eeprom_presets_list(self, api):
        """GET /api/dac/eeprom/presets returns preset list from HAL DB."""
        resp = api.get("/api/dac/eeprom/presets")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_eeprom_parsed_fields(self, api):
        ...
        data = api.get("/api/dac/eeprom").json()
...
class TestDacEepromValidation:
    ...
    def test_eeprom_program_missing_fields(self, api):
        """POST /api/dac/eeprom with empty body should fail gracefully."""
        resp = api.post("/api/dac/eeprom", json={})
