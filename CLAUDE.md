...
- **http_security** — HTTP security header helper. `server_send()` wrapper automatically adds `X-Frame-Options: DENY` and `X-Content-Type-Options: nosniff` to all HTTP responses. All handler files use `server_send()` instead of `server.send()`. `sendGzipped()` applies headers independently for compressed HTML. Lives in `src/http_security.h`
...
- **websocket subsystem** — Real-time state broadcasting to web clients (port 81, `MAX_WS_CLIENTS=16`). Split across 4 focused files sharing the public API in `websocket_handler.h`:
  ...
  Token-based auth (60s TTL, 16-slot pool). Binary frames for waveform (`WS_BIN_WAVEFORM=0x01`) and spectrum (`WS_BIN_SPECTRUM=0x02`); JSON for state. Client-count adaptive binary rate: skip factor 1→8 combined with heap pressure gate. Uses vendored arduinoWebSockets v2.7.3 (`lib/WebSockets/`)
