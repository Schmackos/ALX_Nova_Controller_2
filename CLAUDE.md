...
- **i2s_audio** — Port-generic I2S driver managing 3 I2S ports via unified `I2sPortState _port[3]` array. Public API: `i2s_port_enable_tx/rx()`, `i2s_port_disable_tx/rx()`, `i2s_port_write()`, `i2s_port_read()`, `i2s_port_tdm_read()`, `i2s_port_get_info()` (returns `I2sPortInfo` struct). All 3 ports are equally configurable for STD or TDM mode, any direction (TX/RX), any pins. Legacy device-specific functions (`i2s_audio_write_dac()`, `i2s_configure_adc1/2()`, expansion TX stubs) are deprecated with comments pointing to port-generic replacements. `i2s_audio_init()` delegates audio task creation to `audio_pipeline_init()`. Analysis shared state written by pipeline, read by consumers via `i2s_audio_get_analysis()`. Lives in `src/i2s_audio.h/.cpp`
...
**Port-generic public API**: `i2s_port_enable_tx/rx(port, cfg)`, `i2s_port_disable_tx/rx(port)`, `i2s_port_write(port, buf, len)`, `i2s_port_read(port, buf, len)`, `i2s_port_tdm_read(port, buf, len)`, `i2s_port_get_info(port)` (returns `I2sPortInfo` with mode, direction, pins, clock master, sample rate).
