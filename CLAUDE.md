...
### Quality Gates (CI/CD)
All 5 gates must pass before firmware build proceeds (parallel execution):
1. `cpp-tests` — `pio test -e native -v` (~1697 Unity tests across 71 modules)
...
5. `doc-coverage` — diagram drift prevention checks
...
GitHub Actions (`.github/workflows/tests.yml`): 5 parallel quality gates (cpp-tests, cpp-lint, js-lint, e2e-tests, doc-coverage) must all pass before firmware build. Triggers on push/PR to `main` and `develop` branches. A separate `release.yml` workflow runs the same 5 gates before release. Pipeline diagram: `docs-internal/architecture/ci-quality-gates.mmd`. Playwright HTML report uploaded as artifact on failure (14-day retention).
