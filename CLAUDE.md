...
- **HAL slot capacity**: 14/32 onboard slots at boot + up to 2 expansion (ADC+DAC) = 16/32 (`HAL_MAX_DEVICES=32`). Driver registry: 39/48 (`HAL_MAX_DRIVERS=48`). Device DB: 39/48 (`HAL_DB_MAX_ENTRIES=48`). Pin tracking: `HAL_MAX_PINS=56` with `HAL_GPIO_MAX=54` validation
