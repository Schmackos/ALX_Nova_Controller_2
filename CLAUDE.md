...
- **auth_handler** — Web password auth with PBKDF2-SHA256 (50k iterations, `p2:` format). Backward-compatible with legacy `p1:` (10k) and SHA256 hashes — auto-migrated on login. `PBKDF2_ITERATIONS` constant in `config.h`. First-boot random password on TFT/serial. Rate limiting returns HTTP 429. Cookie `HttpOnly`. WS auth via short-lived token from `GET /api/ws-token` (60s TTL, 16-slot pool). Lives in `src/auth_handler.cpp`
- **http_security** — HTTP security header helper. Adds `X-Frame-Options: DENY` and `X-Content-Type-Options: nosniff` to all HTTP responses via inline `http_add_security_headers()`. Called from `sendGzipped()`, auth handlers, API handlers, and `onNotFound()`. Lives in `src/http_security.h`
...
- **hal_discovery** — 3-tier device discovery: I2C bus scan → EEPROM probe → manual config. Bus scan covers address range 0x08–0x77. Skips Bus 0 (GPIO 48/54) when WiFi active (SDIO conflict) via `hal_wifi_sdio_active()` helper (checks `connectSuccess || connecting || activeInterface == NET_WIFI`); Bus 1 (ONBOARD) and Bus 2 (EXPANSION) always safe. Post-boot rescan via `POST /api/hal/scan` returns `partialScan` flag when Bus 0 is skipped. Emits `DIAG_HAL_I2C_BUS_CONFLICT` (0x1101) on Bus 0 skip. Targeted probe retry for I2C timeout addresses (error codes 4/5): up to `HAL_PROBE_RETRY_COUNT` (2) retries with `HAL_PROBE_RETRY_BACKOFF_MS` (50ms) increasing backoff. Emits `DIAG_HAL_PROBE_RETRY_OK` (0x1105) on retry success. Lives in `src/hal/hal_discovery.h/.cpp`
...
- **hal_ess_sabre_dac_base** — Abstract base class for ESS SABRE DAC family expansion drivers. Provides shared I2C helpers (`_writeReg`, `_readReg`, `_writeReg16`), Wire selection (`_selectWire`), config override reading (`_applyConfigOverrides`), expansion I2S TX lifecycle (`_enableI2sTx`/`_disableI2sTx`), and shared `buildSink()` implementation with static dispatch table. Lives in `src/hal/hal_ess_sabre_dac_base.h/.cpp`
...
- **hal_tdm_interleaver** — TDM frame combiner for multi-channel DACs. Ping-pong buffers combine 4 stereo sink pairs into 8-slot TDM frames for I2S TX. "Last writer flushes" design (pair 3 triggers DMA write). Supports 2 concurrent instances. Thread-safe on Core 1. Lives in `src/hal/hal_tdm_interleaver.h/.cpp`
...
- **websocket_handler** — Real-time state broadcasting to web clients (port 81, `MAX_WS_CLIENTS=16`). Token-based auth (60s TTL, 16-slot pool). Binary frames for waveform (`WS_BIN_WAVEFORM=0x01`) and spectrum (`WS_BIN_SPECTRUM=0x02`); JSON for state. `sendAudioChannelMap()` discovers active sources via HAL. Client-count adaptive binary rate: skip factor 1 (1 client), 2 (2 clients), 4 (3-4 clients), 6 (5-7 clients), or 8 (8+ clients) combined with heap pressure gate. `wsClientCount`/`wsClientMax` fields in `hardwareStats` broadcast. UI warning toast when approaching limit. Auth count recalibrated every `WS_AUTH_RECOUNT_INTERVAL_MS` (10s). `wsAuthenticatedClientCount()` getter exposed in header. Lives in `src/websocket_handler.cpp`
...
**Expansion DACs — 2ch** (5 devices, Pattern C, base class `HalEssSabreDacBase`): ES9038Q2M (128dB, ultra-low power), ES9039Q2M (130dB, Hyperstream IV flagship), ES9069Q (130dB + MQA), ES9033Q (122dB, integrated line drivers), ES9020 (122dB, APLL clock recovery). All 2ch stereo with I2C volume/mute/filter control and `buildSink()` integration. Capability flags: `HAL_CAP_MQA` (ES9069Q), `HAL_CAP_LINE_DRIVER` (ES9033Q), `HAL_CAP_APLL` (ES9020).

**Expansion DACs — 8ch** (7 devices, Pattern D, base class `HalEssSabreDacBase` + `HalTdmInterleaver`): ES9038PRO (132dB, HyperStream II flagship), ES9028PRO (124dB, HyperStream II), ES9039PRO/ES9039MPRO (132dB, HyperStream IV flagship, auto-detect variant), ES9027PRO (124dB, HyperStream IV), ES9081 (120dB, 40-QFN), ES9082 (120dB, 48-QFN, ASP2/SIS), ES9017 (120dB, ES9027PRO pin-compatible). Each registers 4 stereo sink pairs via `getSinkCount()`/`buildSinkAt()` + `HalTdmInterleaver`. TDM output on expansion I2S TX. Per-channel 8-bit volume (regs 0x0F-0x16), 7 filter presets, DPLL bandwidth.

Shared family constants in `src/drivers/ess_sabre_common.h`. Per-device register definitions in `src/drivers/es9*_regs.h` files. Mezzanine connector standard: `docs-site/docs/developer/hal/mezzanine-connector.md`.
...
1. `cpp-tests` — `pio test -e native -v` (~2586 Unity tests across 96 modules)
...
- **HAL slot capacity**: 14/32 onboard slots at boot + up to 2 expansion (ADC+DAC) = 16/32 (`HAL_MAX_DEVICES=32`). Pin tracking: `HAL_MAX_PINS=56` with `HAL_GPIO_MAX=54` validation
