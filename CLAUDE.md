...
- **heap_budget** — Lightweight per-subsystem allocation tracker. Fixed-size array of 32 `HeapBudgetEntry` structs (label, bytes, isPsram). No dynamic allocation. Records audio pipeline, DSP, and other subsystem allocations. Exposed via WS `hardwareStats` and REST `/api/diag/snapshot`. Lives in `src/heap_budget.h/.cpp`
...
### Heap Safety & PSRAM
The Waveshare ESP32-P4-WiFi6-DEV-Kit has PSRAM. Audio pipeline float buffers (~66KB) and DSP delay lines (~77KB) are allocated from PSRAM via `heap_caps_calloc(MALLOC_CAP_SPIRAM)` with internal SRAM fallback. DMA buffers (~32KB) must be internal SRAM. Actual worst-case footprint: ~330KB PSRAM + 32KB internal SRAM.
...
Threshold constants defined in `config.h`: `HEAP_WARNING_THRESHOLD` (50000) and `HEAP_CRITICAL_THRESHOLD` (40000). Each transition emits a diagnostic event and signals `EVT_HEAP_PRESSURE`. PSRAM→SRAM fallback emits `DIAG_SYS_PSRAM_ALLOC_FAIL` (previously defined but unused).
...
- **I2C Bus 0 SDIO conflict**: GPIO 48/54 shared with WiFi SDIO — I2C transactions cause MCU reset. HAL discovery skips Bus 0 when WiFi active
