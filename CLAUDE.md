...
- **hal_ess_sabre_adc_base** — Abstract base class for all 9 ESS SABRE ADC family drivers. Provides shared I2C helpers (`_writeReg`, `_readReg`, `_writeReg16`), Wire selection (`_selectWire`), config override reading (`_applyConfigOverrides`), sample rate validation (`_validateSampleRate()`), and protected member fields. All 2ch I2S (Pattern A) and 4ch TDM (Pattern B) expansion ADC drivers inherit from this class. Lives in `src/hal/hal_ess_sabre_adc_base.h/.cpp`
...
- **hal_ess_sabre_dac_base** — Abstract base class for ESS SABRE DAC family expansion drivers. Provides shared I2C helpers (`_writeReg`, `_readReg`, `_writeReg16`), Wire selection (`_selectWire`), config override reading (`_applyConfigOverrides`), expansion I2S TX lifecycle via port-generic `i2s_port_enable_tx()`/`i2s_port_disable_tx()`, and shared `buildSink()` implementation with static dispatch table. All DAC drivers use `i2s_port_write()` with port from `HalDeviceConfig.i2sPort`. Lives in `src/hal/hal_ess_sabre_dac_base.h/.cpp`
- **hal_es9038q2m** — ESS ES9038Q2M 2-channel SABRE DAC driver. 128dB DNR, Hyperstream II, 128-step volume (0.5dB), 7 filter presets, PCM up to 768kHz. Ultra-low power 40mW. I2C expansion mezzanine device. Compatible: `"ess,es9038q2m"`. Lives in `src/hal/hal_es9038q2m.h/.cpp`
- **hal_es9039q2m** — ESS ES9039Q2M 2-channel SABRE DAC driver. 130dB DNR, Hyperstream IV (flagship). 128-step volume, 7+ filter presets (includes Hyperstream IV hybrid modes), PCM up to 768kHz, DSD1024. Compatible: `"ess,es9039q2m"`. Lives in `src/hal/hal_es9039q2m.h/.cpp`
- **hal_es9069q** — ESS ES9069Q 2-channel SABRE DAC driver. 130dB DNR, Hyperstream IV + integrated MQA hardware renderer. `setMqaEnabled()`/`isMqaActive()` for MQA control. Compatible: `"ess,es9069q"`. Lives in `src/hal/hal_es9069q.h/.cpp`
- **hal_es9033q** — ESS ES9033Q 2-channel SABRE DAC driver. 122dB DNR, integrated 2Vrms ground-centered line drivers (no external op-amp needed). `setLineDriverEnabled()` for output stage control. Compatible: `"ess,es9033q"`. Lives in `src/hal/hal_es9033q.h/.cpp`
- **hal_es9020_dac** — ESS ES9020 2-channel SABRE DAC driver. 122dB DNR, integrated analog PLL (APLL) for clock recovery. `setApllEnabled()`/`isApllLocked()` for APLL control. Compatible: `"ess,es9020-dac"`. Lives in `src/hal/hal_es9020_dac.h/.cpp`
- **hal_es9038pro** — ESS ES9038PRO 8-channel SABRE DAC driver. HyperStream II flagship, 132dB DNR. 8-bit per-channel volume (regs 0x0F-0x16), 7 filter presets, DPLL, TDM input. Registers 4 AudioOutputSinks (CH1/2, CH3/4, CH5/6, CH7/8) via `HalTdmInterleaver`. Compatible: `"ess,es9038pro"`. Lives in `src/hal/hal_es9038pro.h/.cpp`
- **hal_es9028pro** — ESS ES9028PRO 8-channel SABRE DAC driver. HyperStream II, 124dB DNR. Same register layout as ES9038PRO. Compatible: `"ess,es9028pro"`. Lives in `src/hal/hal_es9028pro.h/.cpp`
- **hal_es9039pro** — ESS ES9039PRO/ES9039MPRO 8-channel SABRE DAC driver. HyperStream IV flagship, 132dB DNR. Auto-detects PRO vs MPRO via chip ID. Compatible: `"ess,es9039pro"` / `"ess,es9039mpro"`. Lives in `src/hal/hal_es9039pro.h/.cpp`
- **hal_es9027pro** — ESS ES9027PRO 8-channel SABRE DAC driver. HyperStream IV, 124dB DNR. Pin-compatible with ES9017. Compatible: `"ess,es9027pro"`. Lives in `src/hal/hal_es9027pro.h/.cpp`
- **hal_es9081** — ESS ES9081 8-channel SABRE DAC driver. HyperStream IV, 120dB DNR, 40-QFN. Compatible: `"ess,es9081"`. Lives in `src/hal/hal_es9081.h/.cpp`
- **hal_es9082** — ESS ES9082 8-channel SABRE DAC driver. HyperStream IV, 120dB DNR, 48-QFN, ASP2/SIS support. Compatible: `"ess,es9082"`. Lives in `src/hal/hal_es9082.h/.cpp`
- **hal_es9017** — ESS ES9017 8-channel SABRE DAC driver. HyperStream IV, 120dB DNR. Pin-compatible with ES9027PRO. Compatible: `"ess,es9017"`. Lives in `src/hal/hal_es9017.h/.cpp`
- **hal_cirrus_dac_base** — Abstract base class for Cirrus Logic MasterHIFI DAC expansion drivers. Provides shared I2C helpers (`_writeReg()` for 8-bit regs, `_writeReg16()` for 16-bit paged regs), Wire selection (`_selectWire`), config override reading (`_applyConfigOverrides`), expansion I2S TX lifecycle, and shared `buildSink()` implementation with static dispatch table. Lives in `src/hal/hal_cirrus_dac_base.h/.cpp`
- **hal_cs43198** — Cirrus Logic CS43198 2-channel MasterHIFI DAC driver. 130dBA DNR, 7 filter presets, 128-step volume (0.5dB), DSD256, up to 384kHz/32-bit. Compatible: `"cirrus,cs43198"`. Lives in `src/hal/hal_cs43198.h/.cpp`
- **hal_cs43131** — Cirrus Logic CS43131 2-channel MasterHIFI DAC + headphone amplifier driver. 127dB DNR, 5 filter presets, DSD256, `HAL_CAP_HP_AMP`. `setHeadphoneAmpEnabled(bool)`/`isHeadphoneAmpEnabled()`. 16-bit paged registers. Compatible: `"cirrus,cs43131"`. Lives in `src/hal/hal_cs43131.h/.cpp`
- **hal_cs4398** — Cirrus Logic CS4398 classic 2-channel DAC driver. 120dB DNR, 8-bit registers, max 192kHz/24-bit, DSD64. Compatible: `"cirrus,cs4398"`. Lives in `src/hal/hal_cs4398.h/.cpp`
- **hal_cs4399** — Cirrus Logic CS4399 2-channel MasterHIFI DAC driver. 130dBA DNR, 5 filter presets (including NOS bypass mode), up to 384kHz/32-bit. `setNosMode(bool)`/`isNosMode()`. Compatible: `"cirrus,cs4399"`. Lives in `src/hal/hal_cs4399.h/.cpp`
- **hal_cs43130** — Cirrus Logic CS43130 2-channel MasterHIFI DAC + headphone amp + NOS filter driver. 130dB DNR, 5 filter presets, DSD128, `HAL_CAP_HP_AMP`. Combines HP amp (`setHeadphoneAmpEnabled()`) and NOS mode (`setNosMode()`). 16-bit paged registers. Compatible: `"cirrus,cs43130"`. Lives in `src/hal/hal_cs43130.h/.cpp`
...
- **websocket subsystem** — Real-time state broadcasting to web clients (port 81, `MAX_WS_CLIENTS=16`). Split across 4 focused files sharing the public API in `websocket_handler.h`:
  ...
  Token-based auth (60s TTL, 16-slot pool). Incoming text messages capped at 4096 bytes (DoS prevention). Binary frames for waveform (`WS_BIN_WAVEFORM=0x01`) and spectrum (`WS_BIN_SPECTRUM=0x02`); JSON for state. Client-count adaptive binary rate: skip factor 1→8 combined with heap pressure gate. Uses vendored arduinoWebSockets v2.7.3 (`lib/WebSockets/`)
...
**Expansion DACs — 2ch Cirrus Logic** (5 devices, Pattern C, base class `HalCirrusDacBase`): CS43198 (130dBA, MasterHIFI 7-filter flagship, DSD256), CS43131 (127dB, DAC + integrated headphone amp, DSD256, `HAL_CAP_HP_AMP`), CS4398 (120dB, classic legacy, 8-bit regs, max 192kHz/24-bit), CS4399 (130dBA, 5-filter NOS mode, `setNosMode()`), CS43130 (130dB, HP amp + NOS mode + DSD128). All 2ch stereo with I2C control, 128-step volume (0.5dB), and `buildSink()` integration. I2C address 0x48 (CS4398: 0x4C), I2C Bus 2. Capability flags: `HAL_CAP_HP_AMP` (CS43131, CS43130), `HAL_CAP_DSD` (CS43198, CS43131, CS43130). CS43131/CS43130/CS4399 use 16-bit paged register addressing; CS43198/CS4398 use 8-bit register addressing. Shared constants in `src/drivers/cirrus_dac_common.h`. Per-device registers in `src/drivers/cs4*_regs.h` / `src/drivers/cs43*_regs.h`. Lives in `src/hal/hal_cirrus_dac_base.h/.cpp` + `src/hal/hal_csXXXXX.h/.cpp`.
...
**REST API**: 15 endpoints via `registerHalApiEndpoints()`. All endpoints require `requireAuth()` (session cookie + 30 req/sec rate limit per IP). Core CRUD: GET/POST/PUT/DELETE on `/api/hal/devices`. Discovery: `POST /api/hal/scan` (async — returns 202, 409 guard), `POST /api/hal/devices/reinit`. Database: `GET /api/hal/db`, `GET /api/hal/db/presets`. Settings: `GET|PUT /api/hal/settings` (auto-discovery toggle). Custom schemas: GET/POST/DELETE on `/api/hal/devices/custom`, `POST /api/hal/devices/custom/create`. Unmatched addresses: `GET /api/hal/scan/unmatched`. Config persisted to `/hal_config.json`. Full reference: `docs-site/docs/developer/api/rest-hal.md`.
...
1. `cpp-tests` — `pio test -e native -v` (~3464 Unity tests across 121 modules)
