...
### Heap Safety & PSRAM
The Waveshare ESP32-P4-WiFi6-DEV-Kit has PSRAM. Audio pipeline float buffers (~66KB) and DSP delay lines (~77KB) are allocated from PSRAM via `heap_caps_calloc(MALLOC_CAP_SPIRAM)` with internal SRAM fallback. DMA buffers (16 × 2KB = 32KB) are eagerly pre-allocated from internal SRAM at boot via `heap_caps_calloc(MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA)` — before WiFi connects. Actual worst-case footprint: ~330KB PSRAM + 32KB internal SRAM.
