...
void migrateWiFiCredentials();  // One-time migration from LittleFS to Preferences
bool connectToStoredNetworks(); // Try all saved networks in order
bool saveWiFiNetwork(const char* ssid, const char* password); // Add/update network
...
void handleWiFiScan();
void handleWiFiList();   // GET /api/wifilist - return all saved networks (no passwords)
