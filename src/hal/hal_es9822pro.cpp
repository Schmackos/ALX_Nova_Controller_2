...
HalEs9822pro::HalEs9822pro() : HalAudioDevice() {
...
bool HalEs9822pro::_writeReg(uint8_t reg, uint8_t val) {
#ifndef NATIVE_TEST
    ...
    if (err != 0) {
        LOG_E("[HAL:ES9822PRO] I2C write failed: reg=0x%02X val=0x%02X err=%d", reg, val, err);
        return false;
    }
    return true;
#else
...
// ES9822PRO multi-byte: write LSB at regLsb, MSB at regLsb+1 (MSB write latches both).
bool HalEs9822pro::_writeReg16(uint8_t regLsb, uint16_t val) {
    bool ok  = _writeReg(regLsb,     (uint8_t)(val & 0xFF));
    ok       = ok && _writeReg(regLsb + 1, (uint8_t)((val >> 8) & 0xFF));
    return ok;
}

uint8_t HalEs9822pro::_readReg(uint8_t reg) {
#ifndef NATIVE_TEST
    ...
    LOG_E("[HAL:ES9822PRO] I2C read failed: reg=0x%02X", reg);
    return 0xFF;
#else
    (void)reg;
    return 0x00;
