...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload,
  ...
  case WStype_TEXT: {
    ...
    if (doc["type"] == "toggle") {
      ...
      sendHardwareStats();
    } else if (doc["type"] == "setDebugLevel") {
      int level = doc["level"].as<int>();
      if (level >= LOG_DEBUG && level <= LOG_ERROR) {
        DebugOut.setLogLevel((LogLevel)level);
        DebugOut.printf("Debug level set to: %d (%s)\n", level,
                        DebugOut.getLogLevel() == LOG_DEBUG  ? "DEBUG"
                        : DebugOut.getLogLevel() == LOG_INFO ? "INFO"
                        : DebugOut.getLogLevel() == LOG_WARN ? "WARN"
                                                             : "ERROR");
      }
    }
  } break;
...
void updateCpuUsage() {
  ...
  // We need to calibrate this - typically idle hook runs many times per ms when
  // idle A good estimate is about 10000-50000 iterations per second when fully
  // idle We'll use a dynamic max based on the highest count seen
  ...
  if (idlePerSecond0 > maxIdlePerSecond0)
    maxIdlePerSecond0 = idlePerSecond0;
  if (idlePerSecond1 > maxIdlePerSecond1)
    maxIdlePerSecond1 = idlePerSecond1;
  ...
  if (cpuUsageCore0 < 0)
    cpuUsageCore0 = 0;
  if (cpuUsageCore0 > 100)
    cpuUsageCore0 = 100;
  if (cpuUsageCore1 < 0)
    cpuUsageCore1 = 0;
  if (cpuUsageCore1 > 100)
    cpuUsageCore1 = 100;
}

float getCpuUsageCore0() { return cpuUsageCore0; }

float getCpuUsageCore1() { return cpuUsageCore1; }
