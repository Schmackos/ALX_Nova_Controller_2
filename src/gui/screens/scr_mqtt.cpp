...
static void on_mqtt_enable_confirm(int int_val, float, int) {
    AppState::getInstance().mqtt.enabled = (int_val != 0);
    saveMqttSettings();
...
static void on_mqtt_port_confirm(int int_val, float, int) {
    AppState::getInstance().mqtt.port = int_val;
    saveMqttSettings();
    AppState::getInstance().markSettingsDirty();
...
static void on_mqtt_ha_confirm(int int_val, float, int) {
    AppState::getInstance().mqtt.haDiscovery = (int_val != 0);
    saveMqttSettings();
    AppState::getInstance().markSettingsDirty();
    if (int_val && AppState::getInstance().mqtt.connected) {
        publishHADiscovery();
    }
...
static void on_broker_done(const char *text) {
    AppState::getInstance().mqtt.broker = text;
    saveMqttSettings();
    AppState::getInstance().markSettingsDirty();
...
static void on_username_done(const char *text) {
    AppState::getInstance().mqtt.username = text;
    saveMqttSettings();
    AppState::getInstance().markSettingsDirty();
...
static void on_password_done(const char *text) {
    AppState::getInstance().mqtt.password = text;
    saveMqttSettings();
    AppState::getInstance().markSettingsDirty();
...
static void on_topic_done(const char *text) {
    AppState::getInstance().mqtt.baseTopic = text;
    saveMqttSettings();
    AppState::getInstance().markSettingsDirty();
...
static void edit_mqtt_enable(void) {
    ...
    cfg.type = VE_TOGGLE;
    cfg.toggle_val = AppState::getInstance().mqtt.enabled;
...
static void edit_mqtt_port(void) {
    ...
    cfg.title = "MQTT Port";
    cfg.type = VE_NUMERIC;
    cfg.int_val = AppState::getInstance().mqtt.port;
    cfg.int_min = 1;
    cfg.int_max = 65535;
    cfg.int_step = 1;
...
static void edit_mqtt_ha(void) {
    ...
    cfg.type = VE_TOGGLE;
    cfg.toggle_val = AppState::getInstance().mqtt.haDiscovery;
...
static void build_mqtt_menu(void) {
    ...
    snprintf(enable_str, sizeof(enable_str), "%s", st.mqtt.enabled ? "ON" : "OFF");
    ...
    snprintf(port_str, sizeof(port_str), "%d", st.mqtt.port);
    ...
    snprintf(ha_str, sizeof(ha_str), "%s", st.mqtt.haDiscovery ? "ON" : "OFF");
...
void scr_mqtt_refresh(void) {
    ...
    scr_menu_set_item_value(2, st.mqtt.enabled ? "ON" : "OFF");
    ...
    snprintf(port_buf, sizeof(port_buf), "%d", st.mqtt.port);
    ...
    scr_menu_set_item_value(8, st.mqtt.haDiscovery ? "ON" : "OFF");
}
